...
export class EntitlementController {
  ...
  @ApiOperation({
    summary: 'Check if a specific feature is allowed for this school',
  })
  ...
  @ApiOperation({
    summary: 'Send and track a Whatsapp reminder message (metered limit check)',
  })
  @Post('whatsapp/send')
  async sendWhatsappReminder(
    ...
    if (!check.isAllowed) {
      throw new BadRequestException(
        `WhatsApp Reminder limit exceeded: ${check.reason}. ${
          check.quotas
            ? `Consumed: ${check.quotas.consumedUnits}/${check.quotas.limitCeiling}`
            : ''
        }`,
      );
    }
    ...
    await this.entitlementService.logUsageEvent(
      schoolId,
      'WHATSAPP_REMINDERS',
      1,
      {
    ...
    return {
      ...
      remainingUnits:
        check.quotas?.remainingUnits !== null &&
        check.quotas?.remainingUnits !== undefined
