...
export class StudentCredentialsController {
  ...
  async getCredential(@Param('studentId') studentId: string) {
    ...
    if (!student || !student.studentCode) {
      throw new NotFoundException(
        'Credentials not generated or student not found',
      );
    }
    return {
      id: student.id,
  ...
  async getSchoolStudentCredential(
    ...
    const student = await this.dataSource.getRepository(Student).findOne({
      where: [{ id: studentId, schoolId }, { id: studentId }],
    });
    if (!student || !student.studentCode) {
      throw new NotFoundException(
        'Credentials not generated or student not found',
  ...
  async generateCredential(@Body() body: { studentId: string }) {
    ...
    const student = await this.dataSource
      .getRepository(Student)
      .findOne({ where: { id: studentId }, relations: ['school'] });
  ...
  async bulkGenerate(@Body() body: { studentIds: string[] }) {
    ...
    for (const student of students) {
      if (!student.studentCode) {
        ...
        student.passwordHash = await bcrypt.hash(
