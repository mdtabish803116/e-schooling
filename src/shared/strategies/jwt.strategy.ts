...
export class JwtStrategy extends PassportStrategy(Strategy) {
  ...
  async validate(req: any, payload: JwtPayload): Promise<AuthContext> {
    ...
    if (payload.actorType === 'school_owner') {
      ...
      if (owner.currentSessionToken && rawToken && owner.currentSessionToken !== rawToken) {
        throw new UnauthorizedException('Your session has been logged out because you logged in from another device or browser.');
      }
      ...
      if (user.currentSessionToken && rawToken && user.currentSessionToken !== rawToken) {
        throw new UnauthorizedException('Your session has been logged out because you logged in from another device or browser.');
      }
      ...
      if (student.currentSessionToken && rawToken && student.currentSessionToken !== rawToken) {
        throw new UnauthorizedException('Your session has been logged out because you logged in from another device or browser.');
      }
